...
export class WorkflowGenerator {
            ...
            if (entry.isDirectory()) {
                ...
                try {
                    ...
                    if (skillContent) {
                        ...
                        const workflowContent = this.createAgentOSMarkdown(skillName, skillContent, skillDir);
                        
                        let fileName = skillName;
                        ...
                        console.warn(`Skipping ${skillName}: No recognized documentation found.`);
                    }
                } catch (error) {
     ...
     */
    private createAgentOSMarkdown(folderName: string, rawContent: string, skillDir: string): string {
        ...
        const multiLineMatch = rawContent.match(/description:\s*[|>].*?\r?\n((?:[ \t]+.*\r?\n?)+)/i);
        if (multiLineMatch && multiLineMatch[1]) {
             ...
             const descMatch = rawContent.match(/description:\s*["']?(.*?)["']?(\r?\n|$)/i);
             // Ensure we didn't match the start of a block without capturing body (if regex failed above)
             if (descMatch && descMatch[1] && !descMatch[1].trim().startsWith('|') && !descMatch[1].trim().startsWith('>')) {
